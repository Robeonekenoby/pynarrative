...
class Story:
    ...
    def add_call_to_action(self, 
        ...
        **kwargs):
        """
        Adds a call-to-action element to the visualisation with multiple customisation options.
        Now supports the use of NextStep with any parameter name via kwargs.
        """
        ...
        if next_step_instance is not None:
            return self.add_call_to_action(**next_step_instance.to_dict())
        ...
        if button is not None:
            ...
            return self.add_call_to_action(**button.to_dict())
            ...
            return self.add_call_to_action(**line.to_dict())
    ...
    def add_annotation(self, x_point, y_point, annotation_text="Punto di interesse", 
        ...
        # Inizializza la lista dei layer per l'annotazione
        layers = []

        # Aggiunge il punto se richiesto
        if show_point:
        ...
        arrow_layer = alt.Chart(annotation_data).mark_text(
            text=arrow_symbol, 
            fontSize=arrow_size,
            dx=arrow_dx,  # Offset orizzontale per posizionare la freccia              era  22
            dy=arrow_dy,  # Offset verticale per posizionare la freccia               era -22
            color=arrow_color
        ).encode(
            x=create_encoding('x', x_type),
            y=create_encoding('y', y_type)
        ...
        # Aggiunge l'etichetta di testo
        label_layer = alt.Chart(annotation_data).mark_text(
            align='left',
            baseline='top',
            dx= label_dx,  # Offset orizzontale per posizionare il testo          era 37
            dy= label_dy,  # Offset verticale per posizionare il testo           era -37
            fontSize=label_size,
            color=label_color,
        ...
        # Combina tutti i layer in un'unica annotazione
        annotation = alt.layer(*layers)

        # Aggiunge l'annotazione ai layer della storia
        self.story_layers.append({
            'type': 'annotation',
            'chart': annotation
        })

        # Nota sulla flessibilità e robustezza:
        # Questo approccio rende il metodo add_annotation più flessibile,
        # in quanto può adattarsi automaticamente a diversi tipi di grafici senza
        # richiedere input manuali sul tipo di dati degli assi. Inoltre, utilizzando
        # lo shorthand di Altair, il codice si adatta automaticamente anche se l'utente
        # ha specificato l'encoding in modi diversi (ad esempio, usando alt.X('colonna:Q')
        # o alt.X('colonna', type='quantitative')).

        # Potenziali miglioramenti futuri:
        # - Aggiungere gestione degli errori nel caso in cui lo shorthand non sia
        #   nel formato previsto o il tipo di dato non sia riconoscibile.
        # - Implementare opzioni per posizionamento dinamico delle annotazioni per
        #   evitare sovrapposizioni con elementi esistenti del grafico.
        # - Offrire più opzioni di personalizzazione per l'aspetto delle annotazioni.

        return self  # Ritorna self per permettere il method chaining
    ...
    def _get_position(self, position, is_source=False):
        ...
        """
        # Dizionario che mappa le posizioni alle coordinate (x, y)
        # Le coordinate sono calcolate in base alle dimensioni del grafico
        positions = {
            'left': (10, self.chart.height / 2),  # 10 pixel dal bordo sinistro, centrato verticalmente
            'right': (self.chart.width - 10, self.chart.height / 2),  # 10 pixel dal bordo destro, centrato verticalmente
            'top': (self.chart.width / 2, 80),  # Centrato orizzontalmente, 80 pixel dall'alto
            'bottom': (self.chart.width / 2, self.chart.height - 10),  # Centrato orizzontalmente, 10 pixel dal basso
            'center': (self.chart.width / 2, self.chart.height / 2),  # Centro del grafico
            'side-left': (-10, self.chart.height / 2),  # 10 pixel a sinistra del bordo, centrato verticalmente
            'side-right': (self.chart.width + 10, self.chart.height / 2),  # 10 pixel a destra del bordo, centrato verticalmente
        }
        
        # Se la posizione richiesta non è nel dizionario, usa una posizione di default
        # In questo caso, centrato orizzontalmente e 20 pixel dal basso
    ...
    def create_title_layer(self, layer):
        ...
        title_chart = alt.Chart(self.chart.data).mark_text(
            ...
            x=alt.value(self.chart.width / 2),  # Centra orizzontalmente
            y=alt.value(20)  # Posiziona 20 pixel dall'alto
        )
        ...
        if layer['subtitle']:
            subtitle_chart = alt.Chart(self.chart.data).mark_text(
                ...
                x=alt.value(self.chart.width / 2),  # Centra orizzontalmente
                y=alt.value(50)  # Posiziona 50 pixel dall'alto (sotto il titolo)
            )
    ...
    def create_text_layer(self, layer):
        ...
        return alt.Chart(self.chart.data).mark_text(
            ...
            baseline='middle',
            font=self.font,
            angle=270 if layer.get('vertical', False) else 0,  # Ruota il testo se 'vertical' è True
            color=layer['color']
        ).encode(
            x=alt.value(x),
            y=alt.value(y)
    ...
    def configure_view(self, *args, **kwargs):
        """
        Configura aspetti della vista del grafico utilizzando il metodo configure_view di Altair.

        Questo metodo permette di configurare vari aspetti della vista del grafico, come
        il colore di sfondo, lo stile del bordo, la spaziatura interna, ecc.

        Parametri:
        *args, **kwargs: Argomenti da passare al metodo configure_view di Altair.

        memorizza la configurazione della vista pe applicarla durante il rendering.
        """
    ...
    def render(self):
        ...
        """
        # Iniziamo con il grafico base
        main_chart = self.chart
        
        # Creiamo liste separate per posizionare i grafici speciali
        top_charts = []
        bottom_charts = []
        left_charts = []
        right_charts = []
        ...
        # Organizziamo i layer in base alla loro posizione
        for layer in self.story_layers:
            if layer['type'] == 'special_cta':
                # Prendiamo la posizione dal layer
        ...
        # Sovrapponiamo i layer sul grafico principale
        for overlay in overlay_charts:
            main_chart += overlay

        # Costruiamo il layout finale
        if left_charts:
